...
function injectAuthCenteringStyle() {
  ...
  style.textContent = `
    ...
    @keyframes floatMoney {
      ...
      10%  { opacity: 0.15; }
      90%  { opacity: 0.15; }
      100% { transform: translateY(-50vh) rotate(720deg) scale(1); opacity: 0; }
    }
    ...
    .auth-card {
      ...
      border-radius: 28px;
      padding: 28px 24px;
      box-shadow: 0 20px 35px var(--shadow-color), 0 0 0 1px var(--card-border);
      transition: transform 0.3s ease, box-shadow 0.3s ease;
    }

    .auth-card:hover {
      transform: translateY(-3px);
      box-shadow: 0 25px 40px var(--shadow-color), 0 0 0 2px var(--card-border);
    }
    ...
    .logo-text-container { text-align: center; }
    ...
    @media (max-width: 640px) {
      .auth-card { padding: 20px 18px; max-width: 95%; }
      ...
      .logo-sub-text { font-size: 0.75rem; }
      .form-grid { gap: 12px; margin-bottom: 12px; }
      .btn-primary { padding: 11px 18px; font-size: 0.9rem; }
      .auth-link { font-size: 0.8rem; }
      .auth-logo-wrapper { margin-bottom: 16px; gap: 8px; }
    }

    @media (max-width: 480px) {
      .auth-card { padding: 18px 16px; }
      .logo-circle { width: 70px; height: 70px; overflow: hidden; padding: 0; }
      .logo-image { width: 100%; height: 100%; object-fit: cover; }
...
function createMoneyParticlesHTML() {
  const svgIcons = [icons.creditCard, icons.wallet, icons.coin, icons.money].filter(Boolean);
  let html = '';
  for (let i = 0; i < 20; i++) {
    ...
    html += `
      <div class="money-particle" style="left:${left}%;animation-duration:${duration}s;animation-delay:${delay}s;width:${size}px;height:${size}px;">
        ${sizedSvg}
      </div>`;
  }
  return html;
...
function createLogoHTML(subtitle = 'Gestión de préstamos') {
  return `
    <div class="auth-logo-wrapper">
      ...
      <div class="logo-text-container">
        ...
        <div class="logo-sub-text">${subtitle}</div>
      </div>
    </div>
  `;
}
...
export function renderLogin(onSuccess) {
  ...
  page.innerHTML = `
    ...
    <div class="auth-card">
      ${createLogoHTML('Inicia sesión para continuar')}
      ...
      <div style="text-align:center;margin-top:20px;color:var(--text-secondary);font-size:0.85rem;">
        ¿No tienes cuenta? <a id="switchToRegister" class="auth-link">Regístrate</a>
      </div>
    </div>
  `;
  ...
  page.querySelector('#emailGroup')?.replaceWith(inputGroup({ id:'email', label:'Correo electrónico', type:'email', placeholder:'tu@correo.com', required:true, icon:'mail' }));
  page.querySelector('#passwordGroup')?.replaceWith(inputGroup({ id:'password', label:'Contraseña', type:'password', placeholder:'Mínimo 6 caracteres', required:true, icon:'lock' }));

  page.querySelector('#switchToRegister')?.addEventListener('click', (e) => { e.preventDefault(); renderRegister(onSuccess); });
  page.querySelector('#forgotPasswordLink')?.addEventListener('click', (e) => { e.preventDefault(); toast.info('La recuperación de contraseña estará disponible muy pronto'); });
  ...
  form?.addEventListener('submit', async (e) => {
    ...
    else if (!validateEmail(email)) { showFieldError('email', 'Correo inválido');        valid = false; }
    if (!password)                  { showFieldError('password', 'La contraseña es requerida'); valid = false; }
    if (!valid) return;
...
export function renderRegister(onSuccess) {
  ...
  page.innerHTML = `
    ...
    <div class="auth-card">
      ...
      <div style="text-align:center;margin-top:20px;color:var(--text-secondary);font-size:0.85rem;">
        ¿Ya tienes cuenta? <a id="switchToLogin" class="auth-link">Inicia sesión</a>
      </div>
    </div>
  `;
  ...
  page.querySelector('#switchToLogin')?.addEventListener('click', (e) => { e.preventDefault(); renderLogin(onSuccess); });
